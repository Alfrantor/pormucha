...
async function ensureProductionFormulaColumns() {
  const statements = [
    ...
    `ALTER TABLE "ProductionFormula" ADD COLUMN IF NOT EXISTS "targetLiters" DECIMAL(65,30)`,
...
export async function saveProductionFormula(data: {
  ...
  try {
    ...
    let targetLiters = 1;
    let durationDays = 0;
    let teaType = data.teaType?.trim() || null;
    let teaGramsPerLiter = data.teaGramsPerLiter ?? null;
    let sugarGramsPerLiter = data.sugarGramsPerLiter ?? null;
    let yeastPitchRatePercent = data.yeastPitchRatePercent ?? null;
    let brewWaterPercent = data.brewWaterPercent ?? null;
    let flavorJuicePercent = data.flavorJuicePercent ?? null;
    let flavorItemName = data.flavorItemName?.trim() || null;
    let co2GramsPerLiter = data.co2GramsPerLiter ?? null;
    let carbonationMethod = data.carbonationMethod?.trim() || null;
    let f2ConditionDays = data.f2ConditionDays ?? null;
    let phMin = 0;
    let phMax = 0;
    let brixTarget = 0;
    let ttaTarget = 0;
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.$executeRaw`
        ...
        ("id","code","name","recipeType","description","formulaSummary","targetLiters","teaType","teaGramsPerLiter","sugarGramsPerLiter","yeastPitchRatePercent","brewWaterPercent","flavorJuicePercent","flavorItemName","co2GramsPerLiter","carbonationMethod","f2ConditionDays","durationDays","durationHours","phMin","phMax","brixMin","brixMax","temperatureMin","temperatureMax","acidityMin","acidityMax","isActive","updatedByEmail","createdAt","updatedAt")
        VALUES
        (${formulaId}, ${data.code}, ${data.name.trim()}, ${recipeType}, NULL, ${formulaSummary || null}, ${targetLiters}, ${teaType}, ${teaGramsPerLiter}, ${sugarGramsPerLiter}, ${yeastPitchRatePercent}, ${brewWaterPercent}, ${flavorJuicePercent}, ${flavorItemName}, ${co2GramsPerLiter}, ${carbonationMethod}, ${f2ConditionDays}, ${durationDays}, 0, ${phMin}, ${phMax}, ${brixTarget}, ${brixTarget}, ${temperatureMin}, ${temperatureMax}, ${ttaTarget}, ${ttaTarget}, true, ${updatedByEmail}, NOW(), NOW())
        ON CONFLICT ("code")
        DO UPDATE SET
          "name" = EXCLUDED."name",
      ...
      for (const item of itemRows) {
        await tx.$executeRaw`
          ...
          VALUES
          (${randomUUID()}, ${formulaId}, ${stepId}, ${"RAW_MATERIAL"}, NULL, ${item.rawMaterialId}, ${item.quantity}, ${item.freeTextName}, ${item.sharePercent}, ${item.unitOverride}, NULL, ${item.notes}, NOW(), NOW())
        `;
      }
    });
